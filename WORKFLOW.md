...
| 列 | 含义 |
|----|------|
| 直播间 | 同一直播间只在首行显示徽章，便于视觉分组 |
| 主播 | — |
| 出勤天数 | 该月有记录的**不同日期数**（同一天多场次只算 1 天） |
| 班次分布 | `A × 5` = A班出勤5天；**A1/A2 等上下半场已合并为 A**，只统计 A/B/C/D/E |
| 月销售额 | 该月该主播在该直播间的 GSV 合计（不扣退款） |

- 面板自带月份下拉框（默认最新月），**独立于顶部时间筛选**，切换不影响其他面板。
- 支持 📥 导出Excel（文件名 `主播月度出勤统计_<月份>.xlsx`）。
...
| 2026-09-11 | 业绩demo.html 新增「主播月度出勤统计」面板 + 顶部「按月」快捷筛选按钮 |
