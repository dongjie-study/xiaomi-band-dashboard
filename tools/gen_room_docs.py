...
# 商品卡渠道（无实体直播间）
CARD_CHANNELS = [("我司商品卡", "阳光（我司）"), ("良米商品卡", "良米")]
...
def render_service_md():
    ...
    lines += ["", "> **服务商名注**：「机器空间」（对应表原文）对应内部团队名「机械空间」；"
              "「纵横 / 综讯」对应内部团队名「纵横」。二者源自《小米店铺与服务商对应表》，"
              "与内部团队命名略有差异，如有出入请以实际合同/资质为准。", ""]
    return "\n".join(lines)
...
# 解析器读到的服务商名 → 内部团队名。
# 2026-09-24 服务商名更正后（渡云→凝云、乐群→乐畔、阳光雨蔚→阳光），
# 只剩「机器空间/机械空间」「纵横/综讯」两处仍与内部团队名不同。
AGENT_ALIAS = {
    "阳光": "我司",
    "机器空间": "机械空间",
    "纵横/综讯": "纵横",
