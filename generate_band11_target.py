...
# 四渠道首销月目标（按 9.8-9.11 稳态占比拆分：37.3% / 35.5% / 26.1% / 1.1%）
ROOM_TARGETS = [
    ('小米官方手环直播间', 22400),
    ('小米数码旗舰店',     21200),
    ('我司商品卡',         15700),
...
def sheet_by_room(wb, dates, daily):
    ...
    title_row(ws, '分渠道目标与达成情况', ncols,
              '目标按 9.8-9.11 稳态占比拆分（剔除 9.7 开售日峰值）；剩余日均需求 = 剩余台数 ÷ 剩余天数')
...
def sheet_notes(wb, dates, daily):
    ...
    data_days = [d for d in dates if d in daily]
    base = [d for d in data_days if d > START][-4:]  # 9.8-9.11 稳态
    ...
    r += 1
    hdr_cell(ws, r, 1, '渠道')
    hdr_cell(ws, r, 2, '9.8-9.11 台数')
    hdr_cell(ws, r, 3, '稳态占比')
