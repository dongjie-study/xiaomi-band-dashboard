...
def summarize_day(df):
    ...
    comp_rooms = jixie_rooms + zongheng_rooms + zhumeng_rooms + liangmi_rooms
...
def print_comparison(today, yesterday=None):
    ...
    # ===== 团队总览 =====
    team_colors = {'我司': '★', '机械空间': '◆', '纵横': '▲', '凝云': '●', '逐梦': '◇', '良米': '·'}
    print(f"\n  【四队总览】")
    ...
    if '我司' in type_summary:
        o = type_summary['我司']
        other_orders = sum(type_summary[t]['orders'] for t in ['机械空间','纵横','凝云','逐梦','良米'] if t in type_summary)
        print(f"\n  【总结】")
